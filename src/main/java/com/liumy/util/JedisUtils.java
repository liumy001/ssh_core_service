...
public class JedisUtils {
	...
	public static Logger log = Logger.getLogger(JedisUtils.class.getName());
	//public static JedisSentinelPool pool;
	private static JedisPool pool = null;  

	static {
		// 读取配置文件
		ConfigFile cfg = ConfigFileFactory.getInstance().get("redis");

		/*GenericObjectPoolConfig config = new GenericObjectPoolConfig();
		config.setMaxTotal(cfg.getToInteger("maxActive", 400));
		config.setMaxWaitMillis(cfg.getToInteger("maxWait", 2000));
		config.setBlockWhenExhausted(false);
		config.setTimeBetweenEvictionRunsMillis(2000);

		String MASTER_NAME = cfg.getValue("mastername");
		Set<String> sentinels = new HashSet<String>();
		String[] sentinels_config = cfg.getValue("sentinel").split(" ");
		...
		//pool = new JedisSentinelPool(MASTER_NAME, sentinels, config, cfg.getToInteger("readtimeout", 2000));//20000
		...
		if (pool == null) {  
            ...
            pool = new JedisPool(config, cfg.getValue("redis.ip"), Integer.parseInt(cfg.getValue("redis.port")));  
        }  
...
/*	public static Object exec(String method, Object... param) {
	 ...
	 * 
	 * 返回值： key 不存在时，返-2 key 存在但没有设置剩余生存时间时，返-1 否则，以秒为单位，返key
	 * 的剩余生存时间
	 * 
	 * 
	 * @param key
	 ...
	 * 
	 * 返回值： 设置成功返回 1 key 不存在或者不能为 key 设置生存时间比如在低2.1.3 版本Redis 中你尝试更新
	 * key 的生存时，返0
	 * 
	 * @param key
	 * @param seconds
	...
	public static long expireAt(String key, Date expiry) {
		...
		try {
			jedis = getResource();
			// 时间参数UNIX 时间unix
			// timestamp)；是970日（UTC/GMT的午夜）����过的秒数，不考虑闰秒
	...
	/**
	 * key 的设为 value ，当且仅key 不存在 若给定的 key 已经存在，则 SETNX 不做任何动作SETNX 是SET
	 * if Not eXists如果不存在，SET)的简写
	 * 
	 * 返回值： 设置成功，返1 设置失败，返0 
	 * 
	 * @param key
	...
	/**
	 * value 参数覆写(overwrite)给定 key ��存的字符串，从偏移offset ��不存在的 key
	 * 当作空白字符串处理
	 * 
	 ...
	 * 负数偏移量表示从字符串最后开始计数， -1 表示����字符-2 表示倒数第二个，以此类推
	 * 
	 * 返回值： 截取得出的子字符串
	 * 
	 * @param key
	...
	/**
	 * 将给key 的设为 value ，并返回 key 的旧old value)key 存在但不是字符串类型时，返回��错误
	 * 
	...
	/* ==========================对Hash(哈希操作====================== */
	/**
	 * 将哈希表 key 中的field 的设为 value 如果 key 不存在，��新的哈希表被创建并进HSET 操作如果
	 * field 已经存在于哈希表中，旧将被覆盖
	 * 
	 * 返回值： 如果 field 是哈希表中的��新建域，并且值设置成功，返回 1 如果哈希表中field 已经存在且旧值已被新值覆盖，返回 0
	 * 
	 * 
	 * @param key
	 * @param field
	...
	/**
	 * 将哈希表 key 中的field 的设置value ，当且仅当域 field 不存在 若域 field 已经存在，该操作无效如果
	 * key 不存在，��新哈希表被创建并执行 HSETNX 命令
	 * 
	...
	/**
	 * 同时将多field-value (对设置到哈希key 中 此命令会覆盖哈希表中已存在的域 如果 key
	 * 不存在，��空哈希表被创建并执行 HMSET 操作
	 * 
	 * 返回值： 如果命令执行成功，返OK key 不是哈希hash)类型时，返回��错误
	 * 
	 * @param key
	 * @param hash
	 * @return
	 ...
	 * 被当作一个空哈希表来处理，所以对��不存在的 key 进行 HMGET 操作将返回一个只带有 nil 值的表
	 * 
	...
	/**
	 * 返回哈希key 中，��的域和在返回里，紧跟每个域名(field
	 * name)之后是域的(value)，所以返回的长度是哈希表大小的两
	 * 
...
/*	public static long hdel(byte[] key, byte[]... fields) {
	 ...
	 * 不存在，��新的哈希表被创建并执HINCRBY 命令如果field 不存在，那么在执行命令前，域的被初始化0 
	 * 对一个储存字符串值的field 执行 HINCRBY 命令将成一个错误 本操作的值被限制64 bit)有符号数字表示之内
	 * 
	 ...
	 * 返回哈希key 中的��域
	 * 
	 * 返回值： ��包含哈希表中��域的表 key 不存在时，返回一个空表
	 * 
	 * @param key
	 * @return
	 */
	 ...
	 * 返回哈希key 中所有域的
	 * 
	 * 返回值： ��包含哈希表中��值的表 key 不存在时，返回一个空表
	 * 
	 * @param key
	 * @return
	 */
	...
	/* ==========================对Set(集合)操作====================== */
	/**
	 * 将一个或多个 member 元素加入到集key 当中，已经存在于集合member 元素将被忽略假如 key 不存在，则创建一个只包含
	 * member 元素作成员的集合key 不是集合类型时，返回��错误
	 * 
...
/*	public static long sadd(byte[] key, byte[]... members) {
	...
	/**
	 * 移除集合 key 中的��或多member 元素，不存在member 元素会被忽略key 不是集合类型，返回一个错误
	 * 
...
/*	public static long srem(byte[] key, byte[]... members) {
	 ...
	 * 
	 * 返回 只提key 参数时，返回��元素；如果集合为空，返回 nil 如果提供count
	...
	/* ==========================对List(列表)操作====================== */
	/**
	 * 将一个或多个value 插入到列key 的表如果有多value 值，那么各个 value
	 * 值按从左到右的顺序依次插入到表头LPUSH mylist a b c ，列表的值将c b a;
	 * 
...
/*	public static long lpush(byte[] key, byte[]... values) {
	...
	/**
	 * 将 value 插入到列key 的表头，当且仅当 key 存在并且是一个列表key 不存在时LPUSHX 命令��也不做
	 * 
	...
	/**
	 * 将一个或多个value 插入到列key 的表��如果有多value 值，那么各个 value
	 * 值按从左到右的顺序依次插入到表尾：比如对��空列mylist 执行 RPUSH mylist a b c ，得出的结果列表a b c。如
	 * key 不存在，��空列表会被创建并执行 RPUSH 操作key 存在但不是列表类型时，返回一个错误
	 * 
...
/*	public static long rpush(byte[] key, byte[]... values) {
	...
	/**
	 * 将 value 插入到列key 的表尾，当且仅当 key 存在并且是一个列表 RPUSH 命令相反，当 key 不存在时
	 * RPUSHX 命令��也不做
	 * 
	...
	/**
	 * 返回列表 key 的长度 如果 key 不存在，key 被解释为��空列表，返回 0 . 如果 key 不是列表类型，返回一个错误
	 * 
	 * 返回值： 列表 key 的长度
	 * 
	 * @param key
	 ...
	 * 
	 * 下标(index)参数 start stop 都以 0 为底，也就是说，0 表示列表的第��元素，以 1 表示列表的第二个元素，以此类推
...
/*	public static List<String> lrange(String key, long start, long end) {
	 ...
	 * 
	 * count 的可以是以下几种： count > 0 : 从表头开始向表尾搜索，移除与 value 相等的元素，数量count count
	 * < 0 : 从表尾开始向表头搜索，移除与 value 相等的元素，数量count 的绝对count = 0 : 移除表中��
	 * value 相等的
	 * 
...
/*	public static long lrem(String key, long count, String value) {
	 ...
	 * 
	 * index 参数超出范围，或对一个空列表( key 不存进行 LSET 时，返回��错误
	 * 
...
/*	public static boolean lset(String key, long index, String value) {
	...
	/**
	 * 对一个列表进行修trim)，就是说，让列表只保留指定区间内的元素，不在指定区间之内的元素都将被删除举个例子，执行命LTRIM list
	 * 0 2 ，表示只保留列表 list 的前三个元素，其余元素全部删除
	 * 
	 * 下标(index)参数 start stop 都以 0 为底，也就是说，0 表示列表的第��元素，以 1 表示列表的第二个元素，以此类推
	 * 你也可以使用负数下标，以 -1 表示列表的最后一个元素， -2 表示列表的数第二个元素，以此类推 key
	 * 不是列表类型时，返回��错误。超出范围的下标值不会引起错误
	 * 
	 * 返回 命令执行成功时，返回 ok 
	 * 
	 * @param key
...
/*	public static String lindex(String key, long index) {
	 ...
	 * 
	 * pivot 不存在于列表 key 时，不执行任何操作 key 不存在时key 被视为空列表，不执行任何操作如果 key
	 * 不是列表类型，返回一个错误
	 * 
	 * 返回 如果命令执行成功，返回插入操作完成之后，列表的长度 如果没有找到 pivot ，返-1 如果 key 不存在或为空列表，返
	 * 0 
	 * 
	 * @param key
	 * @param where
	 ...
	 * 
	 * 如果某个 member 已经是有序集的成员，那么更新这个 member score 值，并过重新插入这member 元素，来保证
	 * member 在正确的位置上 score 值可以是整数值或双精度浮点数如果 key 不存在，则创建一个空的有序集并执ZADD 操作
	 * key 存在但不是有序集类型时，返回��错误
	 * 
	...
	/**
	 * 将多个member 元素及其 score 值加入到有序key 当中
	 * 
	 * 如果某个 member 已经是有序集的成员，那么更新这个 member score 值，并过重新插入这member 元素，来保证
	 * member 在正确的位置上 score 值可以是整数值或双精度浮点数如果 key 不存在，则创建一个空的有序集并执ZADD 操作
	 * key 存在但不是有序集类型时，返回��错误
...
/*	public static long zadd(byte[] key, Map<byte[], Double> scoreMembers) {
	...
	/**
	 * 移除有序key 中的��或多个成员，不存在的成员将被忽略key 存在但不是有序集类型时，返回��错误
	 * 
...
/*	public static long zrem(byte[] key, byte[]... members) {
	...
	/**
	 * 返回有序key 中， member的score值在minmember和maxmember的score值之默认包括
	 * member的score值等于minmember或maxmemberscore的成员的数量
	 * 
	 * 返回 member的score值在minmember和maxmember的score值之间的成员的数量
	 * 
	 * @param key
	 * @param min
	 * @param max
...
/*	public static long zcount(byte[] key, byte[] minmember, byte[] maxmember) {
	...
	/**
	 * 返回有序key 中，成员 member score 值如果 member 元素不是有序key 的成员，key 不存在，返回
	 * nil 
	 * 
	 ...
	 * 
	 * 可以通过传��负数increment ，让 score 减去相应的，比ZINCRBY key -5 member ，就是让
	 * member score 值减5 key 不存在，member 不是 key 的成员时ZINCRBY key
	 * increment member 等同ZADD key increment member key 不是有序集类型时，返回一个错误
	 ...
	 * 值的成员按字典序(lexicographical order )来排列如果你需要成员按 score 值从大到小)来排列，请使
	 * ZREVRANGE 命令
	 * 
	 * 下标参数 start stop 都以 0 为底，也就是说，0 表示有序集第��成员，以 1 表示有序集第二个成员，以此类推
	 * 你也可以使用负数下标，以 -1 表示����成员-2 表示倒数第二个成员，以此类推超出范围的下标并不会引起错误
	 * 
...
/*	public static Set<String> zrange(String key, long start, long end) {
	 ...
	 * 值从小到大)次序排列具有相同 score 值的成员按字典序(lexicographical
	 * order)来排该属性是有序集提供的，不��额外的计
	 * 
	 * 返回 指定区间内，带有 score 可)的有序集成员的列表
	 * 
	 * @param key
	 ...
	 * 很大时，定位 offset 的操作可能需要遍历整个有序集，此过程��复杂度为 O(N) 时间
	 * 
	 * 返回 指定区间内，带有 score 可)的有序集成员的列表
	 * 
	 * @param key
	...
	/**
	 * 返回有序key 中，�� member的score 值介minmember maxmember 的score 值之包括等于
	 * minmember maxmember 的score )的成员有序集成员按 score 值从小到大)次序排列具有相同
	 * score 值的成员按字典序(lexicographical order)来排该属性是有序集提供的，不��额外的计
	 * 
	 * 返回 指定区间内，带有 score 可)的有序集成员的列表
	 * 
	 * @param key
...
/*	public static Set<String> zrangeByScore(String key, String minmember, String maxmember, int offset, int count) {
	...
	/**
	 * 返回有序key 中， score 值介max min 之间(默认包括等于 max min )的所有的成员。有序集成员score
	 * 值从大到小)的次序排列具有相同 score 值的成员按字典序的reverse lexicographical order )排列
	 * 
	 * 返回 指定区间内，带有 score 可)的有序集成员的列表
	 * 
	 * @param key
...
/*	public static Set<byte[]> zrevrangeByScore(byte[] key, double max, double min, int offset, int count) {
	...
	/**
	 * 返回有序key 中，�� member的score 值介的score 值之包括等于 maxmember minmember
	 * 的score )的成员有序集成员按 score 值从大到小)次序排列具有相同 score 值的成员按字典reverse
	 * lexicographical order )来排该属性是有序集提供的，不��额外的计
	 * 
	 * 返回 指定区间内，带有 score 可)的有序集成员的列表
	 * 
	 * @param key
...
/*	public static Set<String> zrevrangeByScore(String key, String maxmember, String minmember, int offset, int count) {
	...
	/**
	 * 返回有序key 中成member 的排名其中有序集成员按 score 值从小到大)顺序排列排名0 为底，也就是说，
	 * score 值最小的成员排名0 
	 * 
	...
	/**
	 * 返回有序key 中成member 的排名其中有序集成员按 score 值从大到小)排序排名0 为底，也就是说， score
	 * 值最大的成员排名0 
	 * 
	 * 返回 如果 member 是有序集 key 的成员，返回 member 的排名 如果 member 不是有序key 的成员，返回 nil
	 * 
	...
	/**
	 * 移除有序key 中，指定排名(rank)区间内的��成员
	 * 
	 * 区间分别以下标参start stop 指出，包start stop 在内下标参数 start stop 都以 0
	 * 为底，也就是说，0 表示有序集第��成员，以 1 表示有序集第二个成员，以此类推 你也可以使用负数下标，以 -1 表示����成员-2
	 * 表示倒数第二个成员，以此类推
	 * 
	 * 返回 被移除成员的数量
	 * 
...
/*	public static long zremrangeByRank(String key, long start, long end) {
	...
	public static void main(String[] args){
		...
		//JedisUtils.set("k", "1");
		//JedisUtils.expire("k", 60);
		System.out.println(JedisUtils.get("k"));
	}
